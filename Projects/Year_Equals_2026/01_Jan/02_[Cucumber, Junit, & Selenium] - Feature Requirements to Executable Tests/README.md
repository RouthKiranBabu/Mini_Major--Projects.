## 📚 Table of Contents
<details>
<summary><strong>Click to expand</strong></summary>

- 🚀 [Project Title](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#-cucumber-bdd-automation--selenium--java)  
- ⚡ One-Line Project Summary  
- 🎯 Aim / Objective  
- 🔥 Real-World Problem Statement  
- 🧠 [Core Concept Demonstrated – Cucumber BDD + POM](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#-core-concept-demonstrated--cucumber-bdd--page-object-model)  
- 🧪 Test Scenarios Covered  
- 📂 [Actual Automation Code (Feature, Steps, POM, Runner)](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#-actual-automation-code-with-comments)  
- 🧰 [Tech Stack Used](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#%EF%B8%8F-tech-stack-used)  
- 🧩 Cucumber & Selenium Features Utilized  
- 🛠️ [Challenges Faced & Solutions](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#%EF%B8%8F-challenges-faced--solutions)  
- 📂 Project Structure Overview  
- ▶️ [Execution Demo & Documentation](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#%EF%B8%8F-execution-demo---documentation)  
- 🧠 Key Learnings for SDET Role  
- 🔗 [Proof of Work — Support & Connect](https://github.com/RouthKiranBabu/Mini_Major--Projects./tree/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests#-proof-of-work--support--connect)  
- 👨‍💻 Author  

</details>

---

<h1 align="center">🚀 Cucumber BDD Automation – Selenium + Java</h1>

<h3 align="center">
Real-World BDD Automation Framework Using Cucumber, Selenium, Java & Page Object Model
</h3>

---

<h3 align="center">📌 One-Line Project Summary</h3>

<p align="center">
A real-world Cucumber BDD automation framework that converts business-readable Gherkin scenarios into executable Selenium automation using Java, Maven, JUnit, and Page Object Model.
</p>

---

<h3 align="center">🎯 Aim / Objective</h3>

<p align="center">
To demonstrate how real-world UI test scenarios can be automated using Cucumber BDD with Selenium and Java by following industry-standard framework design and Page Object Model.
</p>

---

<h3 align="center">🔥 Real-World Problem Statement</h3>

<p align="center">
In enterprise projects, automation tests must be readable by non-technical stakeholders while still being robust, maintainable, and scalable.  
This project demonstrates how Cucumber BDD bridges the gap between business requirements and technical automation using Gherkin, Step Definitions, and Page Object Model.
</p>

---

<h3 align="center">🧠 Core Concept Demonstrated – Cucumber BDD + Page Object Model</h3>

<p align="center"><b>This project demonstrates:</b></p>

<p align="left">
🧩 Business-readable test cases using Gherkin <br/>
🧩 Mapping Gherkin steps to Java methods <br/>
🧩 Selenium WebDriver automation <br/>
🧩 Page Object Model for maintainability <br/>
🧩 Maven-based test execution & reporting <br/>
</p>

<p align="center">
<b>✅ Clear separation of concerns<br/>
✅ Scalable automation design<br/>
✅ Recruiter-friendly framework</b>
</p>

---

<h3 align="center">🧪 Test Scenarios Covered</h3>

<p align="left">
✔ Browser initialization and setup <br/>
✔ URL navigation and validation <br/>
✔ Page title validation <br/>
✔ Form field input validation <br/>
✔ Button click actions (Submit / Clear) <br/>
✔ Alert handling and text verification <br/>
✔ Browser teardown and cleanup <br/>
</p>

---

<h3 align="center">📂 Actual Automation Code (With Comments)</h3>

<p align="center">
This project provides direct access to the <b>complete working automation framework</b>, including Feature files, Step Definitions, Page Objects, and Test Runner with clean comments and best practices.
</p>

---

### 🔹 Feature File (Business Layer)

<p align="center">
<a href="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/project_id/Features/Form.feature" target="_blank">
  <img src="https://img.shields.io/badge/View%20Cucumber%20Feature%20File-Gherkin%20Steps-6DB33F?style=for-the-badge&logo=cucumber&logoColor=white"/>
</a>
</p>

---

### 🔹 Step Definitions (Glue Layer)

<p align="center">
<a href="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/project_id/src/test/java/stepDefinitions/Steps.java" target="_blank">
  <img src="https://img.shields.io/badge/View%20Step%20Definitions-Java%20Glue%20Code-007396?style=for-the-badge&logo=java&logoColor=white"/>
</a>
</p>

---

### 🔹 Page Object Model (UI Layer)

<p align="center">
<a href="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/project_id/src/test/java/pageObjects/FormPage.java" target="_blank">
  <img src="https://img.shields.io/badge/View%20Page%20Object%20Model-Selenium%20POM-43B02A?style=for-the-badge&logo=selenium&logoColor=white"/>
</a>
</p>

---

### 🔹 Test Runner (Execution Layer)

<p align="center">
<a href="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/project_id/src/test/java/testRunner/testRun.java" target="_blank">
  <img src="https://img.shields.io/badge/View%20Cucumber%20Test%20Runner-JUnit%20Execution-25A162?style=for-the-badge&logo=junit5&logoColor=white"/>
</a>
</p>

---

<p align="center">
<b>🔍 What reviewers will find inside:</b><br/>
✔ Clean Gherkin scenarios<br/>
✔ Parameterized step definitions<br/>
✔ Selenium interactions via POM<br/>
✔ Alert handling & validations<br/>
✔ Industry-standard framework layout
</p>

---

<h3 align="center">🛠️ Tech Stack Used</h3>

<p align="left">
🔹 Java <br/>
🔹 Selenium WebDriver <br/>
🔹 Cucumber BDD <br/>
🔹 Maven <br/>
🔹 JUnit <br/>
🔹 Page Object Model (POM) <br/>
🔹 Eclipse IDE <br/>
</p>

---

<h3 align="center">🧩 Cucumber & Selenium Features Utilized</h3>

<p align="left">
🔹 Gherkin Feature Files <br/>
🔹 Step Definitions with Annotations <br/>
🔹 Parameterized Scenarios <br/>
🔹 PageFactory & @FindBy <br/>
🔹 Alert Handling <br/>
🔹 HTML & JSON Reports <br/>
🔹 dryRun Execution Mode <br/>
</p>

---

<h3 align="center">🛠️ Challenges Faced & Solutions</h3>

<div align="center">

| Challenge Faced | Solution Implemented |
|----------------|---------------------|
| Mapping Gherkin steps to Java | Used dryRun for step generation |
| Code duplication | Implemented Page Object Model |
| Alert handling complexity | Centralized alert logic in POM |
| Maintainability | Clean folder & package structure |

</div>

---

<h3 align="center">📂 Project Structure Overview</h3>

<p align="left">
📁 <code>Features/</code> – Gherkin feature files <br/>
📁 <code>pageObjects/</code> – UI interaction logic <br/>
📁 <code>stepDefinitions/</code> – Step mappings <br/>
📁 <code>testRunner/</code> – Execution configuration <br/>
📁 <code>Drivers/</code> – Browser drivers <br/>
📁 <code>target/</code> – Execution reports <br/>
</p>

---

<h3 align="center">▶️ Execution Demo & 📄 Documentation</h3>

<table align="center">
<tr>
<td align="center" width="50%">

<b>[DaVinci Resolve] - 🎬 Automation Execution GIF</b><br/><br/>
<img src="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/Gif%20and%20Document/Gifile.gif" width="100%"/>

</td>

<td align="center" width="50%">

<b>📘 Cucumber Integration Guide (PDF)</b><br/><br/>
<a href="https://github.com/RouthKiranBabu/Mini_Major--Projects./blob/main/Projects/Year_Equals_2026/01_Jan/02_%5BCucumber%2C%20Junit%2C%20%26%20Selenium%5D%20-%20Feature%20Requirements%20to%20Executable%20Tests/Gif%20and%20Document/%F0%9F%93%98%20Cucumber%20BDD%20Automation%20Guide.pdf" target="_blank">
  <img src="https://img.shields.io/badge/View%20PDF-Cucumber%20Integration%20Guide-red?style=for-the-badge&logo=adobeacrobatreader"/>
</a>

</td>
</tr>
</table>

---

<h3 align="center">🧠 Key Learnings for SDET Role</h3>

<p align="left">
✔ Designing BDD automation frameworks <br/>
✔ Converting business stories into automation <br/>
✔ Page Object Model best practices <br/>
✔ Test execution using JUnit & Maven <br/>
✔ Writing maintainable Selenium automation <br/>
</p>

---

<h3 align="center">🔗 Proof of Work — Support & Connect</h3>

<p align="center">
<strong>If this project added value, please support by <b>liking</b>, <b>commenting (⭐ out of 5)</b>, and <b>sharing</b> the LinkedIn post below 🚀</strong>
</p>

<p align="center">
  <a href="https://www.linkedin.com/posts/routhkiranbabu_im-happy-to-share-this-cucumber-junit-activity-7414259741201428481-VER9?utm_source=share&utm_medium=member_desktop&rcm=ACoAAC0fSW0BCXvPinW6E3cbBZFekfnprC0b-FU" target="_blank">
    <img src="https://img.shields.io/badge/👍%20Like%20|%20💬%20Comment%20|%20🔁%20Share%20the%20LinkedIn%20Post-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a></br>
  <a href="https://github.com/RouthKiranBabu/LeetCode-Using-Java-JavaScript-and-Python/blob/main/Submissions/README.md#-submissions-by-year--month" target="_blank">
    <img src="https://img.shields.io/badge/Upvote%20My%20LeetCode%20Submission-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
  </a>
  <a href="https://routhkiranbabu.github.io/Strava_stats/" target="_blank">
    <img src="https://img.shields.io/badge/Follow%20My%20Live%20Strava%20Stats-FC4C02?style=for-the-badge&logo=strava&logoColor=white"/>
  </a>
  <a href="https://github.com/RouthKiranBabu" target="_blank">
    <img src="https://img.shields.io/badge/Follow%20Me%20on%20GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

<h3 align="center">👨‍💻 Author</h3>

<p align="center">
<b>Routh Kiran Babu</b><br/>
Aspiring SDET | Cucumber & Selenium Automation Engineer
</p>

<p align="center">
⭐ If this repository helped you, don’t forget to star it!
</p>
